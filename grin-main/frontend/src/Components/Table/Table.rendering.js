            ...
            <table className={styles.table} style={tableStyle}>
                <thead style={theadStyle}>
                    <tr style={trStyle}>
                        ...
                        <th style={{ width: '6%' }}>Quantity No.</th>
                        <th style={{ width: '6%' }}>Quantity in Kgs.</th>
                        <th style={{ width: '7%' }}>Price / Piece (₹)</th>
                        <th style={{ width: '5%' }}>Type</th>
                        <th style={{ width: '5%' }}>Discount (%)</th>
                        <th style={{ width: '7%' }}>Total (₹)</th>
                        <th style={{ width: '6%' }}>Received Weight</th>
                        <th style={{ width: '6%' }}>Ordered Weight</th>
                        <th style={{ width: '6%' }}>Billed Weight</th>
                        <th style={{ width: '7%' }}>Weight Diff (KG)</th>
                        <th style={{ width: '12%' }}>Weight Notes</th>
                    </tr>
                </thead>
                        ...
                        .map((row, index) => (
                            <React.Fragment key={index}>
                                <tr style={trStyle}>
                                    ...
                                    <td style={{ width: '6%' }}>{row.quantityNo}</td>
                                    <td style={{ width: '6%' }}>{row.quantityKg}</td>
                                    <td style={{ width: '7%' }}>{row.price ? `₹${parseFloat(row.price).toFixed(2)}` : '-'}</td>
                                    <td style={{ width: '5%' }}>{row.priceType || '-'}</td>
                                    <td style={{ width: '5%' }}>{row.discount !== undefined ? `${row.discount}%` : '0%'}</td>
                                    <td style={{ width: '7%', fontWeight: 'bold' }}>
                                        ₹{calculateTotal(row.quantityNo, row.price)}
                                    </td>
                                    <td style={{ width: '6%' }}>{row.receivedWeight ? `${parseFloat(row.receivedWeight).toFixed(2)} KG` : '-'}</td>
                                    <td style={{ width: '6%' }}>{row.orderedWeight ? `${parseFloat(row.orderedWeight).toFixed(2)} KG` : '-'}</td>
                                    <td style={{ width: '6%' }}>{row.billedWeight ? `${parseFloat(row.billedWeight).toFixed(2)} KG` : '-'}</td>
                                    <td style={{
                                        width: '7%',
